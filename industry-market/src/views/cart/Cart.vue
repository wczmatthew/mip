<!--  -->
<template lang='html'>
  <w-container show-header :show-back="showBack">
    <!-- 顶部栏 -->
    <div slot="header-mid">
      购物车({{totalNum}})
    </div>
    ...
    <w-modal ref="offlinePayModal"></w-modal>
    <!-- 弹窗内容 end -->
  </w-container>
</template>
<script>
...
export default {
  ...
  watch: {
    '$route'(to) {
      if (to.path === '/market' && parseInt(to.query.tab, 10) === 2) {
        // 重新进入购物车页面
        this.hasNext = true;
      }
    },
